...
for /r slang/ %%I in (*.slang) do (
    ...
    findstr /c:"mainCS" "%%I" >nul && (
        %SlangCompExe% -target spirv -profile glsl_450 -preserve-params -stage compute -entry mainCS "%%I" -o "!outname!"_CS.spv || set "errorfound=1"
    ) || (
        %SlangCompExe% -target spirv -profile glsl_450 -preserve-params -stage vertex -entry mainVS "%%I" -o "!outname!"_VS.spv || set "errorfound=1"
        %SlangCompExe% -target spirv -profile glsl_450 -preserve-params -stage fragment -entry mainFS "%%I" -o "!outname!"_FS.spv || set "errorfound=1"
    )
)
